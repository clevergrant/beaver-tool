...
function startLiveView({ port }) {
  ...
  function renderBuildingsRow() {
    ...
    // Reserve space for: " ◄ " (3) + " ► " (3)
    const overhead = 6;
    let budget = cols - overhead;

    // Build items left-to-right, stopping when they no longer fit
    const visible = [];
    for (let i = 0; i < ordered.length; i++) {
      const name = ordered[i];
      const itemWidth = name.length + 2; // "[name]"
      const needed = itemWidth + (visible.length > 0 ? 1 : 0); // +1 for space separator
      ...
      if (i === 0) {
        ...
        visible.push(`${bg}${c.bold}[${name}]${c.reset}`);
      } else {
        visible.push(`${color}[${name}]${c.reset}`);
      }
    }

    return ` ${c.dim}\u25C4${c.reset} ${visible.join(" ")} ${c.dim}\u25BA${c.reset}`;
  }
