...
type DeviceRecord = Record<string, { type: string; on: boolean; [key: string]: unknown }>;
...
grid = new Grid(gridContainer, gridViewport, {
	storageKey: null,
	onLayoutChange: (id: string, pos: { x: number; y: number; w: number; h: number }) => Store.saveLayout(id, pos),
});
...
function loadConfig(): void {
	...
	(document.querySelector(".header-title") as HTMLElement).textContent = Store.getTitle();
...
function createComponentElement(comp: ComponentData, x: number, y: number, w: number, h: number): void {
	...
	if ((comp as any).aspectRatio) gridOpts.aspectRatio = (comp as any).aspectRatio;
...
function updateComponentElement(comp: ComponentData): void {
	...
	if (!gridComp) return;
	const el: HTMLElement = (gridComp.el.querySelector("tb-component") as HTMLElement) || gridComp.el;
	...
	switch (elem.type) {
		...
		case "color-picker":
			...
			el.addEventListener("color-pick", (e: Event) => {
				...
				if (el.surfaceId && el.parentComponent) {
					...
					handleColorPickViaCircuitry(el.parentComponent, el.surfaceId, detail.color);
				}
			});
			break;
		case "rainbow":
		...
		case "camera":
			...
			el.addEventListener("color-batch", (e: Event) => {
				const detail = (e as CustomEvent).detail;
				if (el.surfaceId && el.parentComponent) {
					handleCameraBatchViaCircuitry(el.parentComponent, el.surfaceId, detail.pixels);
				}
			});
			break;
		default:
...
function propagateScreenResolutions(tbComp: any, circuitry: any): void {
	...
	for (const node of nodes) {
		...
		for (const edge of edges) {
			...
			const surfaceEl: any = findSurfaceElement(tbComp, sourceNode.config.surfaceId);
...
function applyDeviceState(devices: DeviceRecord): void {
	...
	for (const [id, comp] of componentData) {
		...
		const tbComp: any = (gridComp.el.querySelector("tb-component") as any) || gridComp.el;
 ...
 */
function propagateDeviceStateViaCircuitry(tbComp: any, circuitry: any, devices: DeviceRecord): void {
	const { nodes, edges } = circuitry;
	if (!nodes.length) return;
	...
	for (const edge of edges) {
		...
		if (targetNode?.type === "surface-label" && targetNode.config?.surfaceManaged) {
			...
			const surfaceEl: any = findSurfaceElement(tbComp, targetNode.config.surfaceId);
			if (!surfaceEl) continue;
 ...
 */
function handleToggleViaCircuitry(tbComp: any, surfaceId: string, on: boolean): void {
	const circuitry = tbComp.circuitryData;
 ...
 */
function persistSurfaceColor(compId: string, surfaceId: string, color: string): void {
	const comp: ComponentData | null = componentData.get(compId) || Store.readComponent(compId);
 ...
 */
async function handleColorPickViaCircuitry(tbComp: any, surfaceId: string, color: string): Promise<void> {
	const circuitry = tbComp.circuitryData;
	if (!circuitry?.nodes?.length || !circuitry?.edges?.length) return;
 ...
 */
function syncSiblingColorSurfaces(tbComp: any, circuitry: any, targetNodeId: string, sourceNodeId: string, color: string): void {
 ...
 */
function handleCameraBatchViaCircuitry(tbComp: any, surfaceId: string, pixels: Array<{ x: number; y: number; color: string }>): void {
	const circuitry = tbComp.circuitryData;
	if (!circuitry?.nodes?.length || !circuitry?.edges?.length) return;
...
async function handleComponentConfigChange(detail: any): Promise<void> {
	const comp: ComponentData | null | undefined = componentData.get(detail.id) || Store.readComponent(detail.id);
	if (!comp) return;
