	...
	switch (elem.type) {
		...
		case "color-picker":
			...
			el.addEventListener("color-pick", (e) => {
				if (el.surfaceId && el.parentComponent) {
					const compId = el.parentComponent.getAttribute("component-id")
					persistSurfaceColor(compId, el.surfaceId, e.detail.color)
 ...
 */
function persistSurfaceColor(compId, surfaceId, color) {
	const comp = componentData.get(compId) || Store.readComponent(compId)
 ...
 */
async function handleColorPickViaCircuitry(tbComp, surfaceId, color) {
	const circuitry = tbComp.circuitryData
	if (!circuitry?.nodes?.length || !circuitry?.edges?.length) return
	...
	for (const edge of circuitry.edges) {
		...
		const targetNode = circuitry.nodes.find(n => n.id === edge.to)
		if (targetNode?.type === "lever" && targetNode.config?.device) {
			const name = targetNode.config.device
			const url = `/api/color/${encodeURIComponent(name)}/${hex}`
			try {
