...
export const COMP_MIN_WIDTH: number = 8;
export const COMP_MIN_HEIGHT: number = 5;
...
export class Grid {
  ...
  _setupDrag(el: HTMLElement, id: string): void {
    ...
    el.addEventListener("mousemove", (e: MouseEvent) => {
      if (this.locked || editorState?.activeComponentId) { el.style.cursor = ""; return; }
    ...
    el.addEventListener("mousedown", (e: MouseEvent) => {
      if (this.locked || editorState?.activeComponentId) return;
      const comp = this.components.get(id);
      if (!comp) return;
