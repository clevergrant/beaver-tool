const { GAME_API } = require("./config");

let _gameApi = GAME_API;
...
async function setColor(name, rrggbb) {
  const res = await fetch(`${_gameApi}/color/${encodeURIComponent(name)}/${rrggbb}`, { method: "POST" });
  if (!res.ok) throw new Error(`Game API returned ${res.status}`);
}
...
module.exports = { setGameApi, getGameApi, fetchDevices, switchOn, switchOff, toggle, setColor, isGameRunning };
